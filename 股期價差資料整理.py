...
if __name__ == "__main__":
    ...
    for file in files:
        ...
        with open(f'stocklist/{file}') as f:

            for line in f.readlines():
                line = line.rstrip()
                stock_index_mapping[line] = index

    all_targets = []
    for symbol in api.Contracts.Futures.keys():
        if not symbol.isalpha():
            continue
        symbol0 = sorted([x.symbol for x in api.Contracts.Futures[symbol]])[0]
        contract0 = api.Contracts.Futures[symbol][symbol0]
        if 'underlying_code' in contract0.keys():
            if len(contract0.underlying_code) == 4:
                print(f'{symbol} {contract0.underlying_code}')
                all_targets.append((symbol, contract0.underlying_code))
